...
const QADashboard = () => {
  ...
  const errorBudgetUsed =
    ((metrics.lintErrors.total + metrics.typeErrors.total) / 500) * 100;
  const testHealthScore =
    (metrics.testResults.passed /
      (metrics.testResults.passed + metrics.testResults.failed)) *
    100;
      ...
      <div className="max-w-7xl mx-auto space-y-6">
        ...
        {errorBudgetUsed > 80 && (
          <Alert className="border-red-500 bg-red-950/20">
            ...
            <AlertDescription>
              Error budget is {errorBudgetUsed.toFixed(1)}% used. Consider
              prioritizing fixes.
            </AlertDescription>
          </Alert>
        )}
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
          <Card className="bg-gray-900 border-gray-800">
            ...
            <CardContent>
              <div className="text-2xl font-bold text-neon-blue">
                {errorBudgetUsed.toFixed(1)}%
              </div>
              <Progress value={errorBudgetUsed} className="mt-2" />
              <p className="text-xs text-gray-400 mt-1">
                {metrics.lintErrors.total + metrics.typeErrors.total} total
                errors
              </p>
            </CardContent>
          </Card>
          ...
          <Card className="bg-gray-900 border-gray-800">
            ...
            <CardContent>
              <div
                className={`text-2xl font-bold ${getStatusColor(testHealthScore, 90)}`}
              >
                {testHealthScore.toFixed(1)}%
              </div>
              <p className="text-xs text-gray-400">
                {metrics.testResults.passed} passed,{" "}
                {metrics.testResults.failed} failed
              </p>
            </CardContent>
          </Card>
          ...
          <Card className="bg-gray-900 border-gray-800">
            ...
            <CardContent>
              ...
              <p className="text-xs text-gray-400">
                {metrics.ciStatus.duration}min build time
              </p>
            </CardContent>
          </Card>
        </div>
        ...
        <Tabs defaultValue="lint" className="w-full">
          <TabsList className="grid w-full grid-cols-4 bg-gray-900">
            <TabsTrigger
              value="lint"
              className="data-[state=active]:bg-neon-blue"
            >
              Lint Errors
            </TabsTrigger>
            <TabsTrigger
          ...
          <TabsContent value="lint" className="space-y-4">
            <Card className="bg-gray-900 border-gray-800">
              <CardHeader>
                ...
                <CardDescription>
                  Code quality issues across workspaces
                </CardDescription>
              </CardHeader>
              <CardContent>
                <div className="space-y-3">
                  {Object.entries(metrics.lintErrors.byModule).map(
                    ([module, count]) => (
                      <div
                        key={module}
          ...
          <TabsContent value="types" className="space-y-4">
            <Card className="bg-gray-900 border-gray-800">
              <CardHeader>
                ...
                <CardDescription>
                  Type safety issues requiring attention
                </CardDescription>
              </CardHeader>
              <CardContent>
                <div className="space-y-3">
                  {Object.entries(metrics.typeErrors.byWorkspace).map(
                    ([workspace, count]) => (
                      <div
                        key={workspace}
                        className="flex items-center justify-between"
          ...
          <TabsContent value="performance" className="space-y-4">
            <Card className="bg-gray-900 border-gray-800">
              <CardHeader>
                ...
                <CardDescription>
                  Build and runtime performance indicators
                </CardDescription>
              </CardHeader>
              <CardContent className="space-y-4">
                <div className="grid grid-cols-2 gap-4">
                ...
                <div className="pt-4 border-t border-gray-800">
                  <p className="text-xs text-gray-400">
                    Last measured:{" "}
                    {new Date(metrics.performance.lastRun).toLocaleString()}
                  </p>
                </div>
              </CardContent>
            </Card>
            ...
            <div className="space-y-3">
              {metrics.typeErrors.total > 250 && (
                <Alert className="border-red-500 bg-red-950/20">
                  <AlertTriangle className="h-4 w-4" />
                  <AlertDescription>
                    High TypeScript error count ({metrics.typeErrors.total}).
                    Focus on apps/dashboard first.
                  </AlertDescription>
                </Alert>
              )}
              {metrics.testResults.coverage < 70 && (
                <Alert className="border-yellow-500 bg-yellow-950/20">
                  <AlertTriangle className="h-4 w-4" />
                  <AlertDescription>
                    Test coverage below target (70%). Add tests for core-agents
                    package.
                  </AlertDescription>
                </Alert>
              )}
              {metrics.testResults.failed > 10 && (
                <Alert className="border-red-500 bg-red-950/20">
                  <AlertTriangle className="h-4 w-4" />
                  <AlertDescription>
                    {metrics.testResults.failed} failing tests. Review and fix
                    broken test cases.
                  </AlertDescription>
                </Alert>
              )}
