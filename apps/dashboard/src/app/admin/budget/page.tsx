...
export default function AdminBudgetPage() {
  ...
  const agentUsageData = monthlyData?.campaignBreakdown?.reduce(
    (acc: Record<string, any>, campaign: any) => {
      Object.entries(campaign.agents).forEach(
        ([agentType, data]: [string, any]) => {
          if (!acc[agentType]) {
        ...
        <div className="flex items-center justify-between">
          <div>
            ...
            <p className="text-slate-400">
              Monitor AI agent spend and manage budget caps
            </p>
          </div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-4 gap-6">
          ...
          <Card className="bg-slate-900/50 border-slate-800 backdrop-blur-sm">
            ...
            <CardContent>
              ...
              <p className="text-xs text-slate-400 mt-1">
                {monthlyData?.utilizationPercentage?.toFixed(1) || "0"}% of
                budget
              </p>
            </CardContent>
          </Card>
                      ...
                      return (
                        ...
                        >
                            ...
                            {topAgent && (
                              <div className="flex items-center gap-2">
                                ...
                                <span className="text-slate-300 text-sm">
                                  {topAgent[0]} ($
                                  {(topAgent[1] as any).cost.toFixed(2)})
                                </span>
                              </div>
                            )}
                          </TableCell>
                          <TableCell>
                            <Badge
                              variant={
                                campaign.cost > 50 ? "destructive" : "default"
                              }
                              className={
                                campaign.cost > 50
                                  ? "bg-red-900"
            ...
            <Card className="bg-slate-900/50 border-slate-800 backdrop-blur-sm">
              ...
              <CardContent>
                <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
                  {Object.entries(agentUsageData || {}).map(
                    ([agentType, data]: [string, any]) => (
                      <Card
                        key={agentType}
              ...
              <CardContent className="space-y-6">
                ...
                <div className="p-6 bg-slate-800/70 rounded-xl border border-slate-600 space-y-4">
                  <div className="flex items-center justify-between">
                    <div className="space-y-1">
                      <Label
                        htmlFor="budget-override"
                        className="text-white font-medium"
                      >
                        Budget Override Control
                      </Label>
                      <p className="text-sm text-slate-400">
                        Temporarily allow agents to run despite exceeding
                        monthly budget
                      </p>
                    </div>
                    <TooltipProvider>
                      <Tooltip>
                        ...
                        <TooltipContent className="bg-slate-800 border-slate-600 text-white max-w-xs">
                          <p>
                            When enabled, agents can execute tasks even when the
                            monthly budget is exceeded. All override executions
                  ...
                  {budgetOverride && (
                    <Alert className="bg-orange-900/20 border-orange-600">
                      <AlertTriangle className="h-4 w-4 text-orange-400" />
                      <AlertDescription className="text-orange-200">
                        ⚠️ Budget override is currently enabled. Agents can
                        execute tasks despite budget limits. All executions are
                        being logged in{" "}
                ...
                <div className="p-4 bg-slate-800/50 rounded-lg border border-slate-700">
                  ...
                  <div className="space-y-2 text-sm text-slate-400">
                    ...
                    <div className="flex items-center gap-2">
                      ...
                      <span>
                        Hard block when budget exceeded (unless override
                        enabled)
                      </span>
                    </div>
                    <div className="flex items-center gap-2">
                ...
                <div className="p-4 bg-gradient-to-r from-purple-900/20 to-blue-900/20 rounded-lg border border-purple-700/50">
                  ...
                  <div className="space-y-1 text-xs font-mono text-slate-300">
                    <div>
                      ALLOW_BUDGET_OVERRIDE={budgetOverride ? "true" : "false"}
                    </div>
                    <div>MAX_MONTHLY_BUDGET=${budgetAmount}</div>
                  </div>
                  <p className="text-xs text-slate-400 mt-2">
                    These values are set dynamically and override environment
                    file settings.
                  </p>
                </div>
              </CardContent>
