...
test.describe("Customer Intelligence Hub", () => {
  test.beforeEach(async ({ page }) => {
    ...
    await page.route(
      "**/api/trpc/customer.getCustomerSentimentStats*",
      async (route) => {
        await route.fulfill({
          status: 200,
    ...
    await page.route(
      "**/api/trpc/customer.getCustomerTickets*",
      async (route) => {
        await route.fulfill({
          status: 200,
          contentType: "application/json",
    ...
    await page.route(
      "**/api/trpc/customer.getCustomerAnalytics*",
      async (route) => {
        await route.fulfill({
          status: 200,
          contentType: "application/json",
  ...
  test("loads customer intelligence hub successfully", async ({ page }) => {
    ...
    await expect(
      page.locator(
        "text=AI-powered sentiment analysis, support insights, and behavior funnels",
      ),
    ).toBeVisible();
  });
  ...
  test("displays customer analytics overview correctly", async ({ page }) => {
    await expect(
      page.locator("text=📊 Customer Intelligence Overview"),
    ).toBeVisible();
  ...
  test("tab navigation works correctly", async ({ page }) => {
    ...
    await expect(
      page.locator('button:has-text("😊 Sentiment")').first(),
    ).toHaveClass(/text-blue/);

    // Test tickets tab
    await page.click('button:has-text("🎫 Support")');
    await expect(
      page.locator("text=Technical Issue: Login problems"),
    ).toBeVisible();
