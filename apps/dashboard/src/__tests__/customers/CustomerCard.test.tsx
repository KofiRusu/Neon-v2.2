...
describe("CustomerCard", () => {
  ...
  it("displays correct engagement level styling", () => {
    const { rerender } = render(
      <CustomerCard customer={mockCustomer} darkMode={false} />,
    );

    expect(screen.getByText("HIGH")).toHaveClass("text-green-400");

    const mediumCustomer = {
      ...mockCustomer,
  ...
  it("formats lifetime value correctly", () => {
    ...
    const { rerender } = render(
      <CustomerCard customer={highValueCustomer} darkMode={false} />,
    );
  ...
  it("shows avatar or initials correctly", () => {
    ...
    const { rerender } = render(
      <CustomerCard customer={customerWithoutAvatar} darkMode={false} />,
    );
  ...
  it("applies dark mode styling correctly", () => {
    const { container } = render(
      <CustomerCard customer={mockCustomer} darkMode={true} />,
    );
  ...
  it("does not show glow effect for lower-value customers", () => {
    ...
    const { container } = render(
      <CustomerCard customer={lowValueCustomer} darkMode={false} />,
    );
  ...
  it("handles different AI predicted actions", () => {
    ...
    actions.forEach((action, index) => {
      ...
      const { rerender } = render(
        <CustomerCard customer={customer} darkMode={false} />,
      );

      expect(screen.getByText(expectedIcons[index])).toBeInTheDocument();
      expect(
        screen.getByText(`🤖 AI Action: ${action.toUpperCase()}`),
      ).toBeInTheDocument();
