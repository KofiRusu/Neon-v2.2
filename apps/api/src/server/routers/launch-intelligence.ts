...
export const launchIntelligenceRouter = createTRPCRouter({
  ...
  getCampaignExecutionVolume: publicProcedure
    ...
    .query(async ({ input }) => {
      ...
      const grouped = metrics.reduce(
        (acc, metric) => {
          ...
          acc[key].agentBreakdown[metric.agentType] =
            (acc[key].agentBreakdown[metric.agentType] || 0) +
      ...
      const totalExecutions = volumeData.reduce(
      ...
      const avgSuccessRate =
        volumeData.length > 0
          ? volumeData.reduce(
              (sum, d) =>
                sum + d.successfulExecutions / (d.totalExecutions || 1),
              0,
            ) / volumeData.length
          : 0;
      ...
      const firstHalfAvg =
        firstHalf.length > 0
          ? firstHalf.reduce((sum, d) => sum + d.totalExecutions, 0) /
            firstHalf.length
          : 0;
      const secondHalfAvg =
        secondHalf.length > 0
          ? secondHalf.reduce((sum, d) => sum + d.totalExecutions, 0) /
            secondHalf.length
          : 0;
    ...
    .query(async ({ input }) => {
      ...
      const dailyTrends = sentimentData.reduce(
        (acc, item) => {
          ...
          acc[day].languages[
            item.language as keyof (typeof acc)[typeof day]["languages"]
          ]++;
          acc[day].sources[item.source] =
            (acc[day].sources[item.source] || 0) + 1;
      ...
      Object.values(dailyTrends).forEach((day: any) => {
        ...
        day.positivePercent =
          day.total > 0 ? (day.positive / day.total) * 100 : 0;
        day.negativePercent =
          day.total > 0 ? (day.negative / day.total) * 100 : 0;
        day.neutralPercent =
          day.total > 0 ? (day.neutral / day.total) * 100 : 0;
      });
      ...
      const avgSentimentScore =
        totalMentions > 0
          ? sentimentData.reduce((sum, item) => sum + item.score, 0) /
            totalMentions
          : 0;
  ...
  getPacingForecast: publicProcedure
    .input(pacingForecastSchema)
    .query(async ({ input }) => {
      ...
      const pacingVariance =
        spendSoFar > 0
          ? ((spendSoFar - plannedSpendToDate) / plannedSpendToDate) * 100
          : 0;
      ...
      const currentRoi =
        spendSoFar > 0 ? performanceData.totalRevenue / spendSoFar : 0;
          ...
          ctr:
            performanceData.totalImpressions > 0
              ? (performanceData.totalClicks /
                  performanceData.totalImpressions) *
                100
              : 0,
          conversionRate:
  ...
  getLaunchOptimizations: publicProcedure
    ...
    .query(async ({ input }) => {
      ...
      const avgRoi =
        pacingData.length > 0
          ? pacingData.reduce((sum, day) => sum + (day.roi || 0), 0) /
            pacingData.length
          : 0;
      ...
      const roiTrend =
        pacingData.length >= 2
          ? ((pacingData[0].roi || 0) -
              (pacingData[pacingData.length - 1].roi || 0)) /
      ...
      if (currentRoi < targetRoi) {
        if (roiTrend < -0.1) {
          ...
          recommendations.push({
            ...
            description:
              "ROI below target. Consider refining audience targeting or ad creative.",
            confidence: 0.6,
  ...
  trackSentiment: publicProcedure
    ...
    .mutation(async ({ input }) => {
      const { text, platform, language, campaignId, source, region, metadata } =
        input;
...
function analyzeSentiment(text: string, language: string) {
  ...
  positiveWords.forEach((word) => {
    const matches = (lowerText.match(new RegExp(word.toLowerCase(), "g")) || [])
      .length;
    score += matches * 0.1;
    totalWords += matches;
  });
