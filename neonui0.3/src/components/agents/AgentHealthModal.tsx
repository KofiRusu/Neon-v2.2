...
export function AgentHealthModal({
  agentId,
  darkMode,
  onClose,
}: AgentHealthModalProps) {
  const { data: agentData } = trpc.agent.getAllAgents.useQuery();
  const { data: healthData } = trpc.agent.getAgentHealth.useQuery({
    id: agentId,
  });
  const { data: logsData } = trpc.agent.getAgentLogs.useQuery({
    id: agentId,
    limit: 10,
  });
              ...
              >
                ...
                <div className="space-y-3">
                  ...
                  <div className="flex justify-between">
                    ...
                    <span
                      className={`capitalize ${darkMode ? "text-white" : "text-gray-900"}`}
                    >
                      {agent.category}
                    </span>
                  </div>
