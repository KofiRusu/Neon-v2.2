...
test.describe("Copilot UI/UX Production Excellence", () => {
  test.describe("Cross-Platform Layout Validation", () => {
    ...
    viewports.forEach(({ name, width, height }) => {
      test(`should render correctly on ${name} (${width}x${height})`, async ({
        ...
        await expect(
          page.getByText("Welcome to NeonHub Copilot"),
        ).toBeVisible();
        ...
        const sessionButtons = page.locator(
          'button:has-text("Session"), button:has-text("Start")',
        );
    ...
    test("should adapt three-panel layout responsively", async ({ page }) => {
      ...
      const startButton = page
        .locator("button")
        .filter({ hasText: /start.*session|new.*session/i })
        .first();
  ...
  test.describe("Framer Motion & Animation Polish", () => {
    ...
    test("should animate quick start cards on hover", async ({ page }) => {
      ...
      const quickStartCard = page
        .locator('div:has-text("Campaign Analysis")')
  ...
  test.describe("Visual Consistency & Design Tokens", () => {
    ...
    test("should have consistent border radius and shadows", async ({
      page,
    }) => {
      await page.goto("/copilot");
    ...
    test("should handle contrast ratios for accessibility", async ({
      ...
      if ((await darkModeToggle.count()) > 0) {
        ...
        if (
          (await darkModeButton.count()) > 0 &&
          (await darkModeButton.isVisible())
        ) {
  ...
  test.describe("Chat Interface Interactions", () => {
    test("should support voice input when available", async ({
      page,
      browserName,
    }) => {
      await page.goto("/copilot");
    ...
    test("should provide message actions for assistant responses", async ({
      page,
    }) => {
      await page.goto("/copilot");
      const startButton = page
