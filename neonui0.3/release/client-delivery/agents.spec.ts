...
test.describe("Agents Page", () => {
  ...
  test("should display agent cards", async ({ page }) => {
    ...
    const agentCards = page
      ...
      .or(
        page
          .locator("div")
          .filter({ hasText: /Content Agent|SEO Agent|Social Agent/ }),
      );
  ...
  test("should allow agent selection and show details", async ({ page }) => {
    ...
    if (await firstAgent.isVisible()) {
      ...
      await expect(
        page.locator("text=Agent Details").or(page.locator("text=Recent Logs")),
  ...
  test("should display agent logs when agent is selected", async ({ page }) => {
    ...
    if (await firstAgent.isVisible()) {
      ...
      const logsTab = page
        ...
        .or(page.locator("text=Recent Logs"));
      if (await logsTab.isVisible()) {
        ...
        await expect(
          page
            ...
            .or(page.locator("text=No logs available")),
        ).toBeVisible({ timeout: 5000 });
      }
    }
  });
