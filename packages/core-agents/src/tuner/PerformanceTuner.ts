...
export class PerformanceTuner {
  ...
  async analyzeSystem(days: number = 30): Promise<SystemWideAnalysis> {
    ...
    const agentProfiles = await Promise.all(
      Object.keys(allMetrics).map((agentId) =>
        this.analyzeAgent(agentId, days),
      ),
    );

    const totalCost = Object.values(allMetrics).reduce(
    ...
    const underperformers = agentProfiles
      .filter(
        (p) => p.overallHealth === "poor" || p.overallHealth === "critical",
      )
      .map((p) => ({
        agentId: p.agentId,
  ...
  private async generateRecommendations(
    ...
    if (metrics.averageCost > this.benchmarks.costPerRun.fair) {
      ...
      recommendations.push({
        type: "cost",
        severity:
          metrics.averageCost > this.benchmarks.costPerRun.poor
            ? "critical"
            : "high",
    ...
    if (metrics.averageExecutionTime > this.benchmarks.executionTime.fair) {
      ...
      recommendations.push({
        type: "performance",
        severity:
          metrics.averageExecutionTime > this.benchmarks.executionTime.poor
            ? "high"
            : "medium",
        ...
        recommendation:
          "Optimize agent logic, implement caching, or consider parallel processing.",
    ...
    if (metrics.successRate < this.benchmarks.successRate.fair) {
      ...
      recommendations.push({
        type: "reliability",
        severity:
          metrics.successRate < this.benchmarks.successRate.poor
            ? "critical"
            : "high",
    ...
    const tokenEfficiency =
      metrics.totalTokens / Math.max(metrics.totalRuns, 1);
    if (tokenEfficiency > this.benchmarks.tokenEfficiency.fair) {
      recommendations.push({
        type: "cost",
        severity:
          tokenEfficiency > this.benchmarks.tokenEfficiency.poor
            ? "high"
            : "medium",
        ...
        recommendation:
          "Refactor prompts to be more concise while maintaining effectiveness.",
  ...
  private calculateHealthScore(metrics: MemoryMetrics): number {
    ...
    const costMultiplier =
      metrics.averageCost / this.benchmarks.costPerRun.good;
    const costPenalty = Math.min(30, Math.max(0, (costMultiplier - 1) * 15));
    score -= costPenalty;

    // Performance weight: 20%
    const perfMultiplier =
      metrics.averageExecutionTime / this.benchmarks.executionTime.good;
    const perfPenalty = Math.min(20, Math.max(0, (perfMultiplier - 1) * 10));
    score -= perfPenalty;

    // Token efficiency weight: 10%
    const tokenEfficiency =
      metrics.averageTokens / Math.max(metrics.totalRuns, 1);
    const tokenMultiplier =
      tokenEfficiency / this.benchmarks.tokenEfficiency.good;
  ...
  private compareToBenchmarks(
    ...
    const avgCost =
      allValues.reduce((sum, m) => sum + m.averageCost, 0) / allValues.length;
    const avgExecTime =
      allValues.reduce((sum, m) => sum + m.averageExecutionTime, 0) /
      allValues.length;
    const avgSuccessRate =
      allValues.reduce((sum, m) => sum + m.successRate, 0) / allValues.length;
    const avgTokens =
      allValues.reduce((sum, m) => sum + m.averageTokens, 0) / allValues.length;

    return {
      costEfficiency: this.compareToAverage(
        metrics.averageCost,
        avgCost,
        "lower_better",
      ),
      ...
      reliability: this.compareToAverage(
        metrics.successRate,
        avgSuccessRate,
        "higher_better",
      ),
  ...
  private analyzeTrends(
    metrics: MemoryMetrics,
  ): AgentPerformanceProfile["trends"] {
    return {
  ...
  private determineTrend(
    ...
    const key =
      trendData[0] && Object.keys(trendData[0]).find((k) => k !== "date");
  ...
  private generateSystemRecommendations(
    ...
    if (highCostAgents.length > 0) {
      recommendations.push({
        ...
        recommendation:
          "Implement centralized cost management and model selection strategy.",
  ...
  private determineSystemHealth(
    ...
    const avgHealthScore =
      profiles.reduce((sum, p) => sum + p.healthScore, 0) / profiles.length;
  ...
  private determineSystemCostTrend(
    ...
    const allCostTrends = Object.values(allMetrics).map((m) =>
      this.determineTrend(m.costTrend),
    );
    const improvingCount = allCostTrends.filter(
      (t) => t === "improving",
    ).length;
    const decliningCount = allCostTrends.filter(
      (t) => t === "declining",
    ).length;
