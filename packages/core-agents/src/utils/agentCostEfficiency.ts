...
export class AgentCostEfficiencyAnalyzer {
  ...
  private calculateAgentMetrics(
    ...
    const totalImpact = logs.reduce(
      (sum, log) => sum + (log.impactScore || 0),
      0,
    );
    const conversions = logs.filter((log) => log.conversionAchieved).length;
    const totalQuality = logs.reduce(
      (sum, log) => sum + (log.qualityScore || 0),
      0,
    );
    const totalRetries = logs.reduce(
    ...
    const costPerImpact =
      avgImpactScore > 0 ? avgCost / avgImpactScore : Infinity;
    const costPerConversion =
      conversions > 0 ? totalCost / conversions : Infinity;
  ...
  private calculateEfficiencyRating(metrics: {
    ...
    const qualityReliabilityScore =
      ...
      (1 - Math.min(metrics.avgRetryCount, 1)) * 0.3;
  ...
  private generateOptimizationRecommendations(metrics: {
    ...
    if (metrics.avgImpactScore < 0.4) {
      ...
      recommendations.push(
        "📊 Add more specific success criteria and feedback loops",
      );
    }
  ...
  async generateOptimizationSuggestions(timeframe?: {
    ...
    for (const metric of metrics) {
      if (
        metric.efficiencyRating === "CRITICAL" ||
        ...
        if (metric.avgRetryCount > 1) {
          suggestions.push({
            ...
            expectedSavings:
              metric.avgCost * metric.avgRetryCount * 0.5 * metric.totalRuns,
