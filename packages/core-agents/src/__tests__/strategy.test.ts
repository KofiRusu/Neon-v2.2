...
const MockedAgentMemoryStore = AgentMemoryStore as jest.MockedClass<
  typeof AgentMemoryStore
>;
const MockedPerformanceTuner = PerformanceTuner as jest.MockedClass<
  typeof PerformanceTuner
>;
...
describe("Campaign Strategy Builder", () => {
  ...
  beforeEach(() => {
    mockMemoryStore = new MockedAgentMemoryStore(
      null as any,
    ) as jest.Mocked<AgentMemoryStore>;
    ...
    strategyPlanner = new CampaignStrategyPlanner(
      mockMemoryStore,
      mockPerformanceTuner,
    );
  ...
  describe("CampaignStrategyPlanner", () => {
    describe("Product Launch Strategy", () => {
      it("should generate a comprehensive product launch strategy for new AI tool", async () => {
        const goal: CampaignGoal = {
          type: "product_launch",
          objective:
            "Launch revolutionary AI productivity tool targeting early adopters",
          kpis: [
          ...
          budget: {
            total: 50000,
            allocation: { ads: 0.4, content: 0.3, social: 0.3 },
          },
        };
        ...
        const audience: CampaignAudience = {
          ...
          persona: {
            ...
            description:
              "Early adopter who values cutting-edge tools for productivity",
            motivations: [
              "efficiency gains",
        ...
        const trendAction = strategy.actions.find(
          (a) => a.agent === "trend-agent",
        );
        const contentAction = strategy.actions.find(
          (a) => a.agent === "content-agent",
        );
    ...
    describe("Retargeting Campaign Strategy", () => {
      it("should generate retargeting campaign for abandoned carts", async () => {
        const goal: CampaignGoal = {
          type: "retargeting",
          objective:
            "Recover abandoned cart conversions with personalized messaging",
          kpis: [
        ...
        const audience: CampaignAudience = {
          ...
          persona: {
            name: "Hesitant Buyer",
            description:
              "Previously interested customer who needs final push to convert",
        ...
        const insightAction = strategy.actions.find(
          (a) => a.agent === "insight-agent",
        );
    ...
    describe("Seasonal Promotion Strategy", () => {
      it("should generate seasonal sale campaign with Gen Z tone", async () => {
        const goal: CampaignGoal = {
          type: "seasonal_promo",
          objective:
            "Black Friday sale targeting Gen Z with authentic, trendy messaging",
          kpis: [
          ...
          budget: {
            total: 30000,
            allocation: { social: 0.5, ads: 0.3, content: 0.2 },
          },
        };
        ...
        const audience: CampaignAudience = {
          ...
          persona: {
            ...
            description:
              "Socially conscious young adult who values authenticity and trends",
            motivations: [
              "self-expression",
              "social status",
              "value for money",
        ...
        const brandAction = strategy.actions.find(
          (a) => a.agent === "brand-voice-agent",
        );
    ...
    describe("B2B Outreach Strategy", () => {
      it("should generate B2B outreach with high trust messaging", async () => {
        const goal: CampaignGoal = {
          type: "b2b_outreach",
          objective:
            "Generate qualified enterprise leads with trust-focused messaging",
          kpis: [
          ...
          budget: {
            total: 25000,
            allocation: { outreach: 0.4, content: 0.3, email: 0.3 },
          },
        };
        ...
        const audience: CampaignAudience = {
          ...
          persona: {
            ...
            description:
              "C-level executive focused on strategic business outcomes",
            motivations: [
              "competitive advantage",
              "cost reduction",
              "risk mitigation",
        ...
        const outreachAction = strategy.actions.find(
          (a) => a.agent === "outreach-agent",
        );
    ...
    describe("Timeline and Dependencies", () => {
      ...
      it("should generate appropriate timeline stages", async () => {
        ...
        expect(new Date(lastStage.endDate)).toBeLessThanOrEqual(
          new Date("2024-01-31"),
        );
      });
    });
  ...
  describe("Strategy Templates", () => {
    ...
    it("should return template recommendations based on criteria", () => {
      ...
      const lowBudgetTemplates = getTemplateRecommendations(
        5000,
        undefined,
        undefined,
      );
      expect(lowBudgetTemplates.every((t) => t.complexity === "simple")).toBe(
        true,
      );

      // Test timeline filtering
      const shortTimelineTemplates = getTemplateRecommendations(
        undefined,
        14,
        undefined,
      );
      expect(
        shortTimelineTemplates.every((t) => t.estimatedDuration <= 14),
      ).toBe(true);

      // Test channel filtering
      const socialTemplates = getTemplateRecommendations(undefined, undefined, [
        "social",
      ]);
      expect(
        socialTemplates.every((t) => t.recommendedChannels.includes("social")),
      ).toBe(true);
    });
  });
  ...
  describe("Strategy Management", () => {
    ...
    it("should manage execution state correctly", async () => {
      ...
      const executionState =
        await strategyManager.initializeExecution("test-execution-1");
      ...
      const updatedState =
        await strategyManager.getExecutionState("test-execution-1");
    ...
    it("should clone strategies correctly", async () => {
      ...
      const cloned = await strategyManager.cloneStrategy(
        "original-1",
        "Cloned Strategy",
      );
  ...
  describe("Integration Tests", () => {
    it("should demonstrate ContentAgent retry with last success context", async () => {
      ...
      expect(mockMemoryStore.getLastSuccessfulRuns).toHaveBeenCalledWith(
        "content-agent",
        5,
      );
    });
    ...
    it("should demonstrate EmailAgent remembering last campaign context", async () => {
      ...
      const emailAction = strategy.actions.find(
        (a) => a.agent === "email-agent",
      );
