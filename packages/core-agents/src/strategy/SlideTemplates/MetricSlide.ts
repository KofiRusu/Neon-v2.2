...
export class MetricSlideTemplate {
  ...
  private static generateChartVisualization(
    config: ChartConfig,
  ...
  private static generateBusinessContext(data: MetricSlideData): string {
    const positiveTrends = data.metrics.filter(
      (m) => m.status === "positive",
    ).length;
  ...
  private static getTopPerformingMetric(
    metrics: MetricItem[],
  ): MetricItem | null {
  ...
  static createROASSlide(
    ...
    const slideData: MetricSlideData = {
      ...
      highlights: [
        ...
        `Best Performing: ${
          roasData.campaigns.reduce((best, current) =>
