...
export class TrendSlideTemplate {
  ...
  private static generateTrendVisualization(
    data: TrendSlideData,
    ...
    const datasets = [
      {
        ...
        borderColor: this.getTrendColor(
          data.primaryTrend.direction,
          themeColors,
        ),
        backgroundColor: this.getTrendColor(
          data.primaryTrend.direction,
          themeColors,
          0.1,
        ),
    ...
    if (data.secondaryTrends) {
      data.secondaryTrends.forEach((trend, index) => {
        datasets.push({
          ...
          borderColor:
            themeColors.secondary[index % themeColors.secondary.length],
    ...
    if (data.predictions) {
      ...
      const projectedValue =
        lastValue * (1 + data.predictions.projectedMagnitude / 100);

      datasets.push({
        label: "Prediction",
        data: [
          ...new Array(primaryData.length - 1).fill(null),
          lastValue,
          projectedValue,
        ],
  ...
  private static createEventAnnotations(
  ...
  private static generateBusinessContext(data: TrendSlideData): string {
    const highImpactInsights = data.insights.filter(
      (i) => i.impact === "high",
    ).length;
  ...
  private static generateRecommendation(data: TrendSlideData): string {
    ...
    if (actionableInsights.length > 0) {
      const topAction = actionableInsights.find(
        (i) => i.impact === "high",
      )?.recommendation;
    ...
    if (
      primaryTrend.direction === "upward" &&
      primaryTrend.strength === "strong"
    ) {
      return "Capitalize on positive trend momentum with increased investment and expansion";
    } else if (
      primaryTrend.direction === "downward" &&
      primaryTrend.strength === "strong"
    ) {
  ...
  static createROASTrendSlide(
    ...
    const direction =
      magnitude > 5 ? "upward" : magnitude < -5 ? "downward" : "stable";
  ...
  static createBrandTrendSlide(
    ...
    const alignmentMagnitude = this.calculateMagnitude(
      brandTrendData.alignmentTrend,
    );
      ...
      insights: [
        {
          ...
          actionable: true,
          recommendation:
            "Continue brand voice optimization and expand to new channels",
        },
      ],
    };
  ...
  private static calculateMagnitude(
    timeSeriesData: { period: string; value: number }[],
  ): number {
