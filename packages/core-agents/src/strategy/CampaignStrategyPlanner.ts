...
export interface CampaignContext {
  ...
  channels: Array<
    "email" | "social" | "ads" | "content" | "seo" | "outreach" | "whatsapp"
  >;
  constraints?: {
...
export class CampaignStrategyPlanner {
  ...
  private async analyzeAgentPerformance(
    ...
    try {
      ...
      const scoredMetrics: Record<
        string,
        MemoryMetrics & { campaignScore: number }
      > = {};
      ...
      console.warn(
        "Failed to analyze agent performance, using defaults:",
        error,
      );
      return {};
    }
  ...
  private async selectOptimalAgents(
    ...
    const requiredAgents = this.getRequiredAgentsForCampaign(
      goal.type,
      context.channels,
    );
    const optionalAgents = this.getOptionalAgentsForCampaign(
      goal.type,
      audience.segment,
    );
   ...
   */
  private getRequiredAgentsForCampaign(
    campaignType: string,
    channels: string[],
  ): string[] {
    ...
    const channelAgents = channels
      .map((channel) => channelRequirements[channel])
      .filter(Boolean);
   ...
   */
  private getOptionalAgentsForCampaign(
    campaignType: string,
    audienceSegment: string,
  ): string[] {
  ...
  private async generateActionSequence(
    ...
    if (selectedAgents.includes("brand-voice-agent")) {
      const dependencies =
        actions.length > 0 ? [`action-${actions.length}`] : [];
      actions.push(
        this.createAgentAction(
          ++actionCounter,
          "brand-voice-agent",
    ...
    if (
      selectedAgents.includes("seo-agent") &&
      context.channels.includes("seo")
    ) {
      actions.push(
        this.createAgentAction(
          ++actionCounter,
    ...
    if (
      selectedAgents.includes("social-agent") &&
      context.channels.includes("social")
    ) {
      actions.push(
        this.createAgentAction(
          ++actionCounter,
    ...
    if (
      selectedAgents.includes("email-agent") &&
    ...
    if (
      selectedAgents.includes("ad-agent") &&
      context.channels.includes("ads")
    ) {
      actions.push(
        this.createAgentAction(
          ++actionCounter,
    ...
    if (
      selectedAgents.includes("outreach-agent") &&
      context.channels.includes("outreach")
    ) {
      actions.push(
        this.createAgentAction(
          ++actionCounter,
    ...
    if (selectedAgents.includes("design-agent")) {
      const contentDependency = actions.find(
  ...
  private async optimizeActionSequence(
    ...
    const optimizedActions = await Promise.all(
      actions.map(async (action) => {
        try {
          const metrics = await this.memoryStore.getAgentMetrics(
            action.agent,
            30,
          );
          const performanceProfile = await this.performanceTuner.analyzeAgent(
            action.agent,
            30,
          );
  ...
  private async calculateStrategyMetrics(
    ...
    const estimatedDuration = Math.max(
      ...this.calculateParallelDuration(actions),
    );

    const brandAlignment =
      actions.reduce((sum, action) => sum + (action.brandScore || 80), 0) /
      actions.length;

    const avgPerformanceScore =
      actions.reduce(
        (sum, action) => sum + (action.performanceScore || 75),
        0,
      ) / actions.length;
    const successProbability = Math.min(
      95,
      avgPerformanceScore * 0.8 + brandAlignment * 0.2,
    );
  ...
  private generateKeywords(goal: CampaignGoal, product?: any): string[] {
    const baseKeywords = goal.objective
      .split(" ")
