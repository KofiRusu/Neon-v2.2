...
export class AdAgent extends AbstractAgent {
  ...
  constructor() {
    ...
    if (!process.env.OPENAI_API_KEY) {
      logger.warn(
        "OPENAI_API_KEY not found. AdAgent will run in limited mode.",
        {},
        "AdAgent",
      );
    }
    ...
    return this.executeWithErrorHandling(payload, async () => {
      ...
      switch (task) {
        case "optimize_campaigns":
          return await this.optimizeAdCampaigns(
            context as AdOptimizationContext,
          );
        case "allocate_budget":
          return await this.optimizeBudgetAllocation(
            context as AdOptimizationContext,
          );
        case "test_creatives":
          return await this.runCreativeABTest(context as AdCampaignContext);
        case "analyze_audience":
          return await this.analyzeAudiencePerformance(
            context as AdCampaignContext,
          );
        case "adjust_bidding":
          return await this.optimizeBiddingStrategy(
            context as AdCampaignContext,
          );
        ...
        case "predict_performance":
          return await this.predictCampaignPerformance(
            context as AdCampaignContext,
          );
   ...
   */
  private async optimizeAdCampaigns(
...
`;
      ...
      const recommendations = await this.parseBudgetRecommendations(
        context,
        aiInsights,
      );
      ...
      logger.error(
        "AI budget optimization failed, using fallback",
        { error },
        "AdAgent",
      );
      return this.fallbackBudgetOptimization(context);
   ...
   */
  private async runCreativeABTest(
    ...
    try {
      ...
      const testResults = await Promise.all(
        creatives.map(async (creative, index) => {
          const analysis = await this.analyzeCreativePerformance(
            creative,
            context,
          );
      ...
      const recommendations = await this.generateCreativeRecommendations(
        testResults,
        context,
      );
    ...
    try {
      ...
      const insights = await Promise.all(
        audienceSegments.map(async (segment) => {
          const performance = await this.analyzeSegmentPerformance(
            segment,
            context,
          );
          const recommendations = await this.generateAudienceRecommendations(
            segment,
            performance,
          );
          ...
          return {
            ...
            recommendations,
            expansion_opportunities: await this.findExpansionOpportunities(
              segment,
              context,
            ),
          };
      ...
      return {
        ...
        data: {
          segments: insights,
          totalAudienceSize: audienceSegments.reduce(
            (sum, s) => sum + s.size,
            0,
          ),
          topPerformingSegment: insights.reduce((best, current) =>
            current.performance.conversions > best.performance.conversions
              ? current
              : best,
          ),
          optimizationOpportunities: insights.flatMap((i) => i.recommendations),
          expansionRecommendations: insights.flatMap(
            (i) => i.expansion_opportunities,
          ),
        },
      };
    } catch (error) {
   ...
   */
  private async optimizeBiddingStrategy(
    context: AdCampaignContext,
  ): Promise<BiddingAdjustment> {
    try {
      const currentPerformance =
        await this.getCurrentBiddingPerformance(context);
      ...
      return {
        newBids: {},
        success: false,
        error: "Bidding optimization failed",
      };
    }
  }
...
`;
      ...
      return {
        ...
        data: {
          ...
          performanceScore:
            this.calculateOverallPerformanceScore(performanceData),
  ...
  private assessReallocationRisk(
    recommendations: any[],
  ): "low" | "medium" | "high" {
    const totalChange =
      recommendations.reduce(
        (sum, r) =>
          Math.abs(r.recommendedBudget - r.currentBudget) / r.currentBudget,
        0,
      ) / recommendations.length;
  ...
  private calculateCreativeScore(analysis: any): number {
    return (
      analysis.metrics.ctr * 30 +
      analysis.metrics.conversions * 2 +
      Math.random() * 20
    );
  }
  ...
  private async generateTestNextSteps(
    results: any[],
    winner: any,
  ): Promise<string[]> {
    return [
