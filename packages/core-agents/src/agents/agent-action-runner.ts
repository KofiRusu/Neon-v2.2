...
export class AgentActionRunner {
  ...
  private activeActions: Map<string, Promise<ActionExecutionResult>> =
    new Map();
  ...
  public startAutoRun(): void {
    ...
    this.runInterval = setInterval(
      ...
      this.config.runInterval! * 60 * 1000,
    ); // Convert minutes to milliseconds

    console.log(
      `AgentActionRunner auto-run started (interval: ${this.config.runInterval} minutes)`,
    );
  }
  ...
  public async runActionChecks(): Promise<ActionRunResult> {
    ...
    try {
      ...
      const triggeredActions = await this.evaluateRules(
        actionRules,
        recentMetrics,
      );
      ...
      const result: ActionRunResult = {
        ...
        errors: executionResults
          ...
          .map((r) => ({
            ...
            error: r.errorDetails || "Unknown error",
          })),
      ...
      try {
        ...
        const relevantMetrics = metrics.filter(
          (metric) =>
            ...
            (!rule.metricSubtype ||
              metric.metricSubtype === rule.metricSubtype) &&
  ...
  private matchesCampaignFilters(
    metric: AgentMetric,
    campaignIds: string[],
  ): boolean {
  ...
  private matchesPlatformFilters(
    metric: AgentMetric,
    platforms: any[],
  ): boolean {
  ...
  private async executeActions(
  ...
  ): Promise<(ActionExecutionResult & { actionType?: string })[]> {
    ...
    for (const [priority, triggerGroup] of priorityGroups) {
      console.log(
        `Executing ${triggerGroup.length} ${priority} priority actions`,
      );

      // Execute actions in parallel (up to maxConcurrentActions)
      const chunks = this.chunkArray(
        triggerGroup,
        this.config.maxConcurrentActions!,
      );
  ...
  private groupTriggersByPriority(
    triggers: ActionTrigger[],
  ): Map<string, ActionTrigger[]> {
    ...
    const sortedEntries = Array.from(groups.entries()).sort(([a], [b]) => {
      ...
      return (
        (priorityOrder[a as keyof typeof priorityOrder] || 5) -
  ...
  private async executeAction(
    ...
    try {
      console.log(
        `Executing action: ${trigger.actionType} for agent: ${trigger.agentType}`,
      );
      ...
      if (!executor) {
        throw new Error(
          `No executor found for action type: ${trigger.actionType}`,
        );
      }
      ...
      if (!isValid) {
        return {
          ...
          errorDetails: "Parameters are not valid for this action type",
      ...
      // Update action log with results
      ...
      console.error(`Error executing action ${trigger.actionType}:`, error);
    ...
    for (let attempt = 0; attempt <= maxRetries; attempt++) {
      try {
        ...
        if (attempt < maxRetries) {
          const delay = baseDelay * Math.pow(backoffMultiplier, attempt);
          console.log(
            `Retrying action execution in ${delay}ms (attempt ${attempt + 1}/${maxRetries})`,
          );
          await this.delay(delay);
        }
      }
  ...
  private generateRunSummary(
    results: (ActionExecutionResult & { actionType?: string })[],
    executionTime: number,
  ): string {
  ...
  private async sendAlerts(result: ActionRunResult): Promise<void> {
    ...
    const alertMessage =
      `AgentActionRunner Alert: ${result.failedActions} actions failed\n` +
  ...
  public async triggerAction(
    ...
    const params: ActionExecutionParams = {
      ...
      campaignId,
      triggerValue: 0,
      threshold: 0, // Default threshold for manual triggers
      actionConfig,
      metadata: {
        manualTrigger: true,
        triggeredAt: new Date().toISOString(),
