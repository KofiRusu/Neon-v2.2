  ...
  describe("Meta tags generation", () => {
    it("should generate meta tags using AI when API key is available", async () => {
      ...
      const input: MetaTagsInput = {
        topic: "SEO Marketing",
        content:
          "This is comprehensive content about SEO marketing strategies and best practices.",
      ...
      expect(result.title).toBe(
        "SEO Marketing Guide | Expert Tips & Strategies",
      );
  ...
  describe("Content analysis and optimization", () => {
    it("should analyze content and provide SEO suggestions", async () => {
      const context: SEOOptimizationContext = {
        content:
          "This is a short piece of content about SEO. SEO is important for websites.",
    ...
    it("should identify keyword positions correctly", async () => {
      ...
      const keywords = await agent.analyzeContent(content, [
        "SEO",
        "optimization",
      ]);

      const seoKeyword = keywords.find((k) => k.keyword === "SEO");
      const optimizationKeyword = keywords.find(
        (k) => k.keyword === "optimization",
      );
...
Implementing proper SEO strategies will significantly improve your website's search engine rankings and organic traffic.`,
  ...
  describe("Technical SEO audit", () => {
    it("should identify missing H1 tags", async () => {
      ...
      const h1Suggestion = result.data.find((s: any) =>
        s.message.includes("H1 heading"),
      );
    ...
    it("should identify images without alt text", async () => {
      const result = await agent.execute({
        ...
        context: {
          url: "https://example.com/page",
          content:
            '<h1>Title</h1><img src="image.jpg"><img src="image2.jpg" alt="Description">',
        },
        priority: "medium",
      });

      const altTextSuggestion = result.data.find((s: any) =>
        s.message.includes("alt text"),
      );
      expect(altTextSuggestion).toBeDefined();
      expect(altTextSuggestion.message).toContain(
