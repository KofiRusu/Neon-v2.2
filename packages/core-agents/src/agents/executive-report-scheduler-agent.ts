...
export class ExecutiveReportSchedulerAgent extends BaseAgent {
  ...
  async execute(
    ...
    try {
      console.log(
        "🕐 ExecutiveReportScheduler: Starting autonomous reporting engine...",
      );
  ...
  private async startAutonomousScheduler(
    config: SchedulerConfig,
  ): Promise<void> {
    ...
    console.log(
      "✅ Autonomous scheduler started - checking every hour for scheduled tasks",
    );
  }

  private async checkAndExecuteScheduledTasks(
    config: SchedulerConfig,
  ): Promise<void> {
    try {
      const now = new Date();
      console.log(
        `🔍 ExecutiveReportScheduler: Checking scheduled tasks at ${now.toISOString()}`,
      );

      // Check for weekly digest generation
      if (
        config.enableWeeklyDigest &&
        this.shouldGenerateWeeklyDigest(now, config)
      ) {
      ...
      if (
        config.enableAgentPerformanceReports &&
        this.shouldGenerateMonthlyAgentReport(now)
      ) {
      ...
      if (
        config.enableBrandAudits &&
        this.shouldGenerateQuarterlyBrandAudit(now)
      ) {
  ...
  private shouldGenerateWeeklyDigest(
    now: Date,
    config: SchedulerConfig,
  ): boolean {
  ...
  private async generateWeeklyDigest(config: SchedulerConfig): Promise<void> {
    try {
      ...
      if (report.success) {
        ...
        console.log(
          `✅ Weekly digest generated successfully: ${report.data.reportId}`,
        );
      }
    } catch (error) {
  ...
  private async checkForCompletedCampaigns(
    config: SchedulerConfig,
  ): Promise<void> {
    ...
    for (const campaign of completedCampaigns) {
      if (!campaign.hasGeneratedSummary) {
        console.log(
          `📈 Generating summary for completed campaign: ${campaign.name}`,
        );
  ...
  private async generateBrandConsistencyAudit(
    config: SchedulerConfig,
  ): Promise<void> {
    try {
  ...
  private async monitorSystemHealth(config: SchedulerConfig): Promise<void> {
    ...
    if (healthMetrics.avgCampaignROAS < config.performanceThresholds.lowROAS) {
      issues.push(
        `Average ROAS below target: ${healthMetrics.avgCampaignROAS}x`,
      );
    }

    if (
      healthMetrics.brandAlignment <
      config.performanceThresholds.criticalBrandAlignment
    ) {
      issues.push(
