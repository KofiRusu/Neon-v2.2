...
describe("EmailMarketingAgent", () => {
  ...
  describe("Email personalization", () => {
    it("should personalize email using AI", async () => {
        ...
        choices: [
          {
            message: {
              content: JSON.stringify({
                personalizedSubject:
                  "Hi John! Your exclusive tech industry update",
                personalizedContent:
      ...
      expect(result.personalizedSubject).toBe(
        "Hi John! Your exclusive tech industry update",
      );
  ...
  describe("Performance analysis", () => {
    ...
    it("should provide optimization suggestions based on metrics", async () => {
      ...
      expect(
        result.optimizationSuggestions.some(
          (s) => s.category === "Subject Lines",
        ),
      ).toBe(true);
      expect(
        result.optimizationSuggestions.some((s) => s.category === "Content"),
      ).toBe(true);
      expect(
        result.optimizationSuggestions.some(
          (s) => s.category === "Deliverability",
        ),
      ).toBe(true);
    });
  });
  ...
  describe("A/B testing", () => {
    ...
    it("should throw error for insufficient variants", async () => {
      ...
      await expect(agent.runABTest(input)).rejects.toThrow(
    ...
    it("should calculate performance metrics for each variant", async () => {
      ...
      result.variants.forEach((variant) => {
        ...
        expect(variant.performance.opens).toBeLessThanOrEqual(
          variant.performance.sent,
        );
        expect(variant.performance.clicks).toBeLessThanOrEqual(
          variant.performance.opens,
        );
        expect(variant.performance.conversions).toBeLessThanOrEqual(
    ...
    it("should identify winner correctly", async () => {
      ...
      result.variants.forEach((variant) => {
        if (variant.id !== winner?.id) {
          expect(winnerMetric).toBeGreaterThanOrEqual(
