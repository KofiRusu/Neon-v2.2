...
export class UIRefinementAgent extends AbstractAgent {
  ...
  private async analyzeContrast(
    ...
    const files =
      parameters.files || (await this.getUIFiles(parameters.targetDir));
  ...
  private async fixContrastIssues(
    ...
    for (const file of files) {
      try {
        ...
        if (issues.length > 0) {
          const { fixedContent, fixed } = this.applyContrastFixes(
            content,
            issues,
          );
  ...
  private async fixSpacingIssues(
    ...
    for (const file of files) {
      try {
        ...
        if (issues.length > 0) {
          const { fixedContent, fixed } = this.applySpacingFixes(
            content,
            issues,
          );
  ...
  private async fixAccessibilityIssues(
    ...
    for (const file of files) {
      try {
        ...
        if (issues.length > 0) {
          const { fixedContent, fixed } = this.applyAccessibilityFixes(
            content,
            issues,
          );
  ...
  private async analyzeResponsive(
    ...
    for (const file of files) {
      try {
        ...
        logger.warn(`Failed to analyze responsive design in ${file}`, {
          error,
        });
      }
    }
  ...
  private async fixResponsiveIssues(
    ...
    for (const file of files) {
      try {
        ...
        if (issues.length > 0) {
          const { fixedContent, fixed } = this.applyResponsiveFixes(
            content,
            issues,
          );
  ...
  private async checkContrastInFile(
    file: string,
    content: string,
  ): Promise<UIIssue[]> {
    const issues: UIIssue[] = [];
      ...
      if (spacingMatch) {
        spacingMatch.forEach((match) => {
          ...
          if (nonStandardSpacing) {
            ...
            const suggestedClass = this.getNearestStandardSpacing(
              currentClass,
              prefix,
            );
  ...
  private getNearestStandardSpacing(
    currentClass: string,
    prefix: string,
  ): string {
