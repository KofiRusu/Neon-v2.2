...
if (fs.existsSync(".github/workflows/validate-workspace.yml")) {
  recordResult(
    "Validation Workflow",
    "PASS",
    "Workspace validation workflow deployed",
  );
} else {
...
try {
  const autoCommits = execSync(
    'git log --oneline --grep="Auto-commit from device" | wc -l',
    {
      encoding: "utf8",
    },
  ).trim();
  ...
  recordResult(
    "Auto-Commit Activity",
    "FAIL",
    "Unable to analyze commit history",
  );
}
...
if (fs.existsSync(".sync-log.json")) {
  try {
    ...
    recordResult(
      "Sync Logging",
      "WARN",
      "Sync log exists but cannot be parsed",
    );
  }
} else {
  recordResult(
...
try {
  ...
  recordResult(
    "Auto-Sync Test",
    "FAIL",
    "Auto-sync script execution failed",
    error.message,
  );
}
...
const successRate = Math.round(
  (validationResults.summary.passed / totalTests) * 100,
);
