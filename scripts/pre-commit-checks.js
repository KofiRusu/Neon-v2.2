...
function getFilesByExtension(files, extensions) {
  return files.filter(
    (file) =>
      extensions.some((ext) => file.endsWith(ext)) && fs.existsSync(file),
  );
}
...
async function main() {
  ...
  const jstsFiles = getFilesByExtension(stagedFiles, [
    ".js",
    ".jsx",
    ".ts",
    ".tsx",
  ]);
  const cssFiles = getFilesByExtension(stagedFiles, [
    ".css",
    ".scss",
    ".module.css",
  ]);
  ...
  if (jstsFiles.length > 0 || cssFiles.length > 0 || jsonFiles.length > 0) {
    try {
      execSync(
        `git add ${[...jstsFiles, ...cssFiles, ...jsonFiles].join(" ")}`,
        { stdio: "pipe" },
      );
    } catch (e) {
