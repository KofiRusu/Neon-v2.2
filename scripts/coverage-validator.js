...
function parseCoverageReport() {
  ...
  if (!fs.existsSync(coveragePath)) {
    console.error(
      '❌ Coverage report not found. Run "npm run test:coverage" first.',
    );
    process.exit(1);
  }
...
function validateAgentCoverage(coverageData) {
  ...
  Object.entries(coverageData).forEach(([filePath, data]) => {
    if (
      filePath.includes("packages/core-agents/src/agents/") &&
      filePath !== "total"
    ) {
...
function generateCoverageReport(
  ...
  console.log(
    `   Branches:   ${total.branches.pct}% (Required: ${COVERAGE_THRESHOLDS.branches}%)`,
  );
  console.log(
    `   Functions:  ${total.functions.pct}% (Required: ${COVERAGE_THRESHOLDS.functions}%)`,
  );
  console.log(
    `   Lines:      ${total.lines.pct}% (Required: ${COVERAGE_THRESHOLDS.lines}%)`,
  );
...
function main() {
  ...
  const passed = generateCoverageReport(
    coverageData,
    globalViolations,
    agentViolations,
  );
