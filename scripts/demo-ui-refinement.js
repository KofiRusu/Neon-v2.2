...
const {
  UIRefinementAgent,
} = require("../packages/core-agents/src/agents/ui-refinement-agent");
...
async function demonstrateUIRefinement() {
  ...
  const agent = new UIRefinementAgent(
    "demo-ui-agent",
    "Demo UI Refinement Agent",
  );
  ...
  try {
    ...
    if (contrastResult.success && contrastResult.data.issues.length > 0) {
      console.log(
        `⚠️  Found ${contrastResult.data.issues.length} contrast issues:`,
      );
      contrastResult.data.issues.forEach((issue, index) => {
        console.log(`  ${index + 1}. ${issue.description}`);
        console.log(`     📍 ${issue.file}:${issue.line}`);
        console.log(
          `     🔧 Suggested fix: ${issue.suggestedValue.substring(0, 100)}...`,
        );
        console.log("");
      });
    } else {
    ...
    if (
      ...
      console.log(
        `⚠️  Found ${accessibilityResult.data.issues.length} accessibility issues:`,
      );
      ...
      if (accessibilityResult.data.issues.length > 5) {
        console.log(
          `     ... and ${accessibilityResult.data.issues.length - 5} more`,
        );
      }
      console.log("");
    } else {
    ...
    if (themeResult.success && themeResult.data.fixedIssues.length > 0) {
      console.log(
        `✨ Fixed ${themeResult.data.fixedIssues.length} theme consistency issues:`,
      );
    ...
    console.log(
      "The agent can also be integrated into your development workflow",
    );
    console.log(
      "to automatically detect and fix UI issues during development.",
    );
  } catch (error) {
