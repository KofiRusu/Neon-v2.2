...
export function CampaignExecutionPanel({
  ...
  const handleAnalyzeCampaign = async (campaignId: string) => {
    try {
      const analysis = await analyzeCampaignMutation.mutateAsync({
        campaignId,
      });
      ...
      <div className="flex items-center justify-between">
        <div>
          ...
          <p className="text-gray-400">
            Real-time campaign monitoring and agent coordination
          </p>
        </div>
  ...
  return (
    <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
      ...
      <Card className="glass-card">
        ...
        <CardContent className="space-y-4">
          {Object.entries(metrics.performance).map(
