...
test.describe('Agent Logs Dashboard', () => {
  ...
  test('should load logs dashboard correctly', async ({ page }) => {
    ...
    await expect(page.locator('text=Monitor and audit all AI agent activity in real-time')).toBeVisible();
  });

  test('should display filters and search', async ({ page }) => {
    await expect(page.locator('text=Filters & Search')).toBeVisible();
    await expect(page.locator('input[placeholder*="Search messages, agents, tasks"]')).toBeVisible();
  ...
  test('should show table and timeline views', async ({ page }) => {
    await expect(page.locator('[role="tab"]:has-text("Table View")')).toBeVisible();
    await expect(page.locator('[role="tab"]:has-text("Timeline View")')).toBeVisible();
  });

  test('should filter logs by search', async ({ page }) => {
    await page.waitForTimeout(1000);
    await page.fill('input[placeholder*="Search messages, agents, tasks"]', 'content');
