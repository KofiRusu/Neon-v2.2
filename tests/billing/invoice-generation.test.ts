...
describe("Invoice Generation System", () => {
  ...
  describe("fetchInvoiceData", () => {
    test("should fetch and process billing data correctly", async () => {
      ...
      const campaign1 = invoiceData.campaignBreakdown.find(
        (c) => c.id === "campaign-1",
      );
      ...
      expect(invoiceData.agentSummary[AgentType.CONTENT].totalExecutions).toBe(
        2,
      );
  ...
  describe("error handling", () => {
    test("should handle database connection errors", async () => {
      mockPrisma.billingLog.findMany.mockRejectedValue(
