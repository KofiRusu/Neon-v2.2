...
class BrandVoiceValidator {
  ...
  private async testSuggestionGeneration(): Promise<void> {
    await this.runTest("Suggestion Generation", async () => {
      ...
      const result = await this.agent.getSuggestionsPublic(
        poorContent,
        "email",
      );
      ...
      for (const suggestion of result.suggestions) {
        ...
        if (
          !["tone", "vocabulary", "structure", "style"].includes(
            suggestion.type,
          )
        ) {
        ...
        if (!["low", "medium", "high"].includes(suggestion.priority)) {
          throw new Error(
            `Invalid suggestion priority: ${suggestion.priority}`,
          );
        }
      }
    });
  ...
  private generateReport(): void {
    ...
    console.log(
      `Average Test Duration: ${Math.round(totalDuration / totalTests)}ms\n`,
    );
    ...
    if (passedTests === totalTests) {
      console.log(
        "🎉 All tests passed! Brand Voice Agent is ready for production.",
      );
    } else {
      console.log(
